...
Latex.new = function()
    local latex = setmetatable({
        reader = 'zathura',
        diretorios = {
            modelos = Diretorio.new(vim.env.HOME) / 'documentos' / 'git' / 'ouvidoria-latex-modelos',
...
---@diagnostic disable-next-line: undefined-field
    local auxiliares = vim.fn.glob((self.diretorios.destino / '*.{aux,out,log}').path, false, true)
...
---@diagnostic disable-next-line: undefined-field
    vim.env.TEXINPUTS = '.:' .. self.diretorios.modelos.path .. ':'
end
...
Latex.compile = function(self)
    ...
    cmd = {
        ...
        '-o',
		---@diagnostic disable-next-line: undefined-field
		self.diretorios.destino.path,
        '-k',
        '-Z',
		---@diagnostic disable-next-line: undefined-field
        'search-path=' .. self.diretorios.modelos.path,
		arquivo
    }
...
---@diagnostic disable-next-line: undefined-field
        local ci = (Ouvidoria.latex.diretorios.destino / titulo).path
...
Cmus.Musicas = Diretorio.new(vim.env.HOME) / 'documentos'/ 'musicas'
...
if vim.fn.filereadable('/home/andre/.config/dmenu_player/pastas_de_musicas.dmenu') == 1 then
	Cmus.Pastas = '/home/andre/.config/dmenu_player/pastas_de_musicas.dmenu'
else
	vim.notify('Não foi possível encontrar o arquivo de listagem dos diretórios.')
end
...
Cmus.diretorios_musica = function()
    return vim.fn.systemlist({'ls', Cmus.Musicas.path})
end
...
Cmus.acoes = {
    ...
    queue = function(dir)
        ...
        dir = vim.fn.fnameescape((Cmus.Musicas / dir).path)
        Cmus.comando('-c', '-q')
        Cmus.comando('-q', dir)
